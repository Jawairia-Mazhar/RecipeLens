...
const RecipeCard = ({ recipe, favorites, addToFav, removeFromFav }) => {
  ...
  return (
    <div className='card cursor-pointer overflow-hidden flex flex-col gap-2 items-center border border-gray-300 rounded-md p-2 w-70'>
      ...
      <div
        ...
        <div className='relative group'>
          ...
          <div className='flex justify-center items-center w-56 h-56 rounded-full absolute inset-0 bg-transparent backdrop-blur-xs bg-opacity-50 opacity-0 group-hover:opacity-90 transition-opacity duration-200'>
            <button 
              onClick={(e) => {            
                e.stopPropagation();
        ...
        <div className='card__text w-full'>
          <h2 className='card__title text-center line-clamp-2'>{recipe.title}</h2>
        </div>
      </div>
    </div>
  )
